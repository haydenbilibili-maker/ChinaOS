...
import { BUSINESS_ELITE_DEDUPED_COUNT } from '../db/businessEliteSeed.js';
import { otKey, OVERSEAS_TALENT_DEDUPED_COUNT } from '../db/overseasTalentSeed.js';
...
export const SEARCH_INDEX_REVISION = `v20:talent-expansion4-2026-06:${ANTI_CORRUPTION_COUNT}:${GLOSSARY_COUNT}:${SELF_MEDIA_DEDUPED_COUNT.total}:${CULTURAL_ELITE_DEDUPED_COUNT.total}:${BUSINESS_ELITE_DEDUPED_COUNT.total}:${OVERSEAS_TALENT_DEDUPED_COUNT.total}:${DIPLOMATIC_CORPS_SEED_PKG?.rows?.length ?? 0}:${DISSIDENT_DEDUPED_COUNT.total}:${TAIWAN_POLITICAL_DEDUPED_COUNT.total}:${FIGURE_SEED_COUNT}`;
