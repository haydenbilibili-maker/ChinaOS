// ============================================================================
// 本地法律原文库 · manifest + fetch
// 文件位于 app/public/legal-corpus/ → 浏览器访问 /legal-corpus/...
// ============================================================================

const CORPUS_BASE = '/legal-corpus';
...
export const CORPUS_STUB_THRESHOLD = 500;

export const CORPUS_TIER_LABELS = {
  full: '本地全文',
  excerpt: '原文节选',
  extended: '扩展汇编',
  stub: '原文节选 · 不完整',
...
export const CORPUS_DISCLAIMER =
  '本地存档仅供研究学习，以全国人大/国务院公布文本为准；引用、执法与合规请以国家法律法规数据库及官方发布渠道为准。';

/** @returns {Promise<{ version: number, generatedAt: string, entries: Record<string, { id: string, title: string, type: string, issuer?: string, effectiveDate?: string, corpusFile?: string, corpusTier?: 'full'|'excerpt' }> }>} */
export async function loadLegalCorpusManifest() {
  if (!_manifestPromise) {
...
/** 同步探测：manifest 已加载时可用 */
export function peekCorpusEntry(id) {
  if (!_manifestPromise) return null;
  // manifest promise resolved state not easily peeked; use async path in UI
  return null;
}
