/** 流动与留守儿童 GY-54 · 页签 / 观测哨(命名空间 chinaos.et.v1) */
import { AS_OF_BASELINE } from '../../lib/config/asOfBaseline.js';
import { ertongPanelId } from '../../lib/ertong/routing.js';
import { withGyInit } from '../shared/gy/enhanceMethodology.js';

const NS = 'chinaos.et.v1';

/** @param {HTMLElement | null} root @param {{ tab?: string }} deepLink */
function initErtongCore(root, deepLink = {}) {
  if (!root) return () => {};

  const cleanups = [];

  const nav = root.querySelectorAll('.et-nav button');
  const activatePanel = (panelId) => {
    nav.forEach((b) => b.classList.toggle('is-active', b.dataset.panel === panelId));
    root.querySelectorAll('section.et-panel').forEach((p) => {
  ...
  const stateEl = root.querySelector('#et-storage-state');
  if (stateEl) {
  ...
  root.querySelectorAll('#et-watch-list .et-watch-item').forEach((item) => {
    const key = item.dataset.key;
    const btns = item.querySelectorAll('.et-status-btn');
    const note = item.querySelector('.et-watch-note');
    const stampEl = item.querySelector('.et-watch-stamp');
    const rec = data[key] || {};
  ...
  activatePanel(ertongPanelId(deepLink.tab));

  return () => cleanups.forEach((fn) => fn());
}

export const initErtong = withGyInit(initErtongCore);
