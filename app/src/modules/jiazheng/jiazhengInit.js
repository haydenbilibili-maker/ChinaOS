/** 家政与照护工人 GY-31 · 页签 / 观测哨(命名空间 chinaos.jz.v1) */
import { AS_OF_BASELINE } from '../../lib/config/asOfBaseline.js';
import { jiazhengPanelId } from '../../lib/jiazheng/routing.js';
import { withGyInit } from '../shared/gy/enhanceMethodology.js';

const NS = 'chinaos.jz.v1';

/** @param {HTMLElement | null} root @param {{ tab?: string }} deepLink */
function initJiazhengCore(root, deepLink = {}) {
  if (!root) return () => {};

  const cleanups = [];

  const nav = root.querySelectorAll('.jz-nav button');
  const activatePanel = (panelId) => {
    nav.forEach((b) => b.classList.toggle('is-active', b.dataset.panel === panelId));
    root.querySelectorAll('section.jz-panel').forEach((p) => {
  ...
  const stateEl = root.querySelector('#jz-storage-state');
  if (stateEl) {
  ...
  root.querySelectorAll('#jz-watch-list .jz-watch-item').forEach((item) => {
    const key = item.dataset.key;
    const btns = item.querySelectorAll('.jz-status-btn');
    const note = item.querySelector('.jz-watch-note');
    const stampEl = item.querySelector('.jz-watch-stamp');
    const rec = data[key] || {};
  ...
  activatePanel(jiazhengPanelId(deepLink.tab));

  return () => cleanups.forEach((fn) => fn());
}

export const initJiazheng = withGyInit(initJiazhengCore);
