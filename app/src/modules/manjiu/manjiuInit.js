/** 慢就业青年/全职儿女/NEET GY-34 · 页签 / 观测哨(命名空间 chinaos.mj.v1) */
import { AS_OF_BASELINE } from '../../lib/config/asOfBaseline.js';
import { manjiuPanelId } from '../../lib/manjiu/routing.js';
import { withGyInit } from '../shared/gy/enhanceMethodology.js';

const NS = 'chinaos.mj.v1';

/** @param {HTMLElement | null} root @param {{ tab?: string }} deepLink */
function initManjiuCore(root, deepLink = {}) {
  if (!root) return () => {};

  const cleanups = [];

  const nav = root.querySelectorAll('.mj-nav button');
  const activatePanel = (panelId) => {
    nav.forEach((b) => b.classList.toggle('is-active', b.dataset.panel === panelId));
    root.querySelectorAll('section.mj-panel').forEach((p) => {
  ...
  const stateEl = root.querySelector('#mj-storage-state');
  if (stateEl) {
  ...
  root.querySelectorAll('#mj-watch-list .mj-watch-item').forEach((item) => {
    const key = item.dataset.key;
    const btns = item.querySelectorAll('.mj-status-btn');
    const note = item.querySelector('.mj-watch-note');
    const stampEl = item.querySelector('.mj-watch-stamp');
    const rec = data[key] || {};
  ...
  activatePanel(manjiuPanelId(deepLink.tab));

  return () => cleanups.forEach((fn) => fn());
}

export const initManjiu = withGyInit(initManjiuCore);
