/** 中小学教师 GY-38 · 页签 / 观测哨(命名空间 chinaos.js.v1) */
import { AS_OF_BASELINE } from '../../lib/config/asOfBaseline.js';
import { jiaoshiPanelId } from '../../lib/jiaoshi/routing.js';
import { withGyInit } from '../shared/gy/enhanceMethodology.js';

const NS = 'chinaos.js.v1';

/** @param {HTMLElement | null} root @param {{ tab?: string }} deepLink */
function initJiaoshiCore(root, deepLink = {}) {
  if (!root) return () => {};

  const cleanups = [];

  const nav = root.querySelectorAll('.js-nav button');
  const activatePanel = (panelId) => {
    nav.forEach((b) => b.classList.toggle('is-active', b.dataset.panel === panelId));
    root.querySelectorAll('section.js-panel').forEach((p) => {
  ...
  const stateEl = root.querySelector('#js-storage-state');
  if (stateEl) {
  ...
  root.querySelectorAll('#js-watch-list .js-watch-item').forEach((item) => {
    const key = item.dataset.key;
    const btns = item.querySelectorAll('.js-status-btn');
    const note = item.querySelector('.js-watch-note');
    const stampEl = item.querySelector('.js-watch-stamp');
    const rec = data[key] || {};
  ...
  activatePanel(jiaoshiPanelId(deepLink.tab));

  return () => cleanups.forEach((fn) => fn());
}

export const initJiaoshi = withGyInit(initJiaoshiCore);
