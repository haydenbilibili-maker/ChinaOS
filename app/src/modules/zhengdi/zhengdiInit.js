/** 征地拆迁与失地农民 GY-51 · 页签 / 观测哨(命名空间 chinaos.zd.v1) */
import { AS_OF_BASELINE } from '../../lib/config/asOfBaseline.js';
import { zhengdiPanelId } from '../../lib/zhengdi/routing.js';
import { withGyInit } from '../shared/gy/enhanceMethodology.js';

const NS = 'chinaos.zd.v1';

/** @param {HTMLElement | null} root @param {{ tab?: string }} deepLink */
function initZhengdiCore(root, deepLink = {}) {
  if (!root) return () => {};

  const cleanups = [];

  const nav = root.querySelectorAll('.zd-nav button');
  const activatePanel = (panelId) => {
    nav.forEach((b) => b.classList.toggle('is-active', b.dataset.panel === panelId));
    root.querySelectorAll('section.zd-panel').forEach((p) => {
  ...
  const stateEl = root.querySelector('#zd-storage-state');
  if (stateEl) {
  ...
  root.querySelectorAll('#zd-watch-list .zd-watch-item').forEach((item) => {
    const key = item.dataset.key;
    const btns = item.querySelectorAll('.zd-status-btn');
    const note = item.querySelector('.zd-watch-note');
    const stampEl = item.querySelector('.zd-watch-stamp');
    const rec = data[key] || {};
  ...
  activatePanel(zhengdiPanelId(deepLink.tab));

  return () => cleanups.forEach((fn) => fn());
}

export const initZhengdi = withGyInit(initZhengdiCore);
