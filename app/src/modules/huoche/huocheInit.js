/** 货车司机与公路货运劳动者 GY-27 · 页签 / 观测哨(命名空间 chinaos.hc.v1) */
import { AS_OF_BASELINE } from '../../lib/config/asOfBaseline.js';
import { huochePanelId } from '../../lib/huoche/routing.js';
import { withGyInit } from '../shared/gy/enhanceMethodology.js';

const NS = 'chinaos.hc.v1';

/** @param {HTMLElement | null} root @param {{ tab?: string }} deepLink */
function initHuocheCore(root, deepLink = {}) {
  if (!root) return () => {};

  const cleanups = [];

  const nav = root.querySelectorAll('.hc-nav button');
  const activatePanel = (panelId) => {
    nav.forEach((b) => b.classList.toggle('is-active', b.dataset.panel === panelId));
    root.querySelectorAll('section.hc-panel').forEach((p) => {
  ...
  const stateEl = root.querySelector('#hc-storage-state');
  if (stateEl) {
  ...
  root.querySelectorAll('#hc-watch-list .hc-watch-item').forEach((item) => {
    const key = item.dataset.key;
    const btns = item.querySelectorAll('.hc-status-btn');
    const note = item.querySelector('.hc-watch-note');
    const stampEl = item.querySelector('.hc-watch-stamp');
    const rec = data[key] || {};
  ...
  activatePanel(huochePanelId(deepLink.tab));

  return () => cleanups.forEach((fn) => fn());
}

export const initHuoche = withGyInit(initHuocheCore);
