/** 高校青椒与过剩博士 GY-33 · 页签 / 观测哨(命名空间 chinaos.qj.v1) */
import { AS_OF_BASELINE } from '../../lib/config/asOfBaseline.js';
import { qingjiaoPanelId } from '../../lib/qingjiao/routing.js';
import { withGyInit } from '../shared/gy/enhanceMethodology.js';

const NS = 'chinaos.qj.v1';

/** @param {HTMLElement | null} root @param {{ tab?: string }} deepLink */
function initQingjiaoCore(root, deepLink = {}) {
  if (!root) return () => {};

  const cleanups = [];

  const nav = root.querySelectorAll('.qj-nav button');
  const activatePanel = (panelId) => {
    nav.forEach((b) => b.classList.toggle('is-active', b.dataset.panel === panelId));
    root.querySelectorAll('section.qj-panel').forEach((p) => {
  ...
  const stateEl = root.querySelector('#qj-storage-state');
  if (stateEl) {
  ...
  root.querySelectorAll('#qj-watch-list .qj-watch-item').forEach((item) => {
    const key = item.dataset.key;
    const btns = item.querySelectorAll('.qj-status-btn');
    const note = item.querySelector('.qj-watch-note');
    const stampEl = item.querySelector('.qj-watch-stamp');
    const rec = data[key] || {};
  ...
  activatePanel(qingjiaoPanelId(deepLink.tab));

  return () => cleanups.forEach((fn) => fn());
}

export const initQingjiao = withGyInit(initQingjiaoCore);
