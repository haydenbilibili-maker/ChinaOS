/** 边疆少数民族 GY-48 · 页签 / 观测哨(命名空间 chinaos.bj.v1) */
import { AS_OF_BASELINE } from '../../lib/config/asOfBaseline.js';
import { bianjiangPanelId } from '../../lib/bianjiang/routing.js';
import { withGyInit } from '../shared/gy/enhanceMethodology.js';

const NS = 'chinaos.bj.v1';

/** @param {HTMLElement | null} root @param {{ tab?: string }} deepLink */
function initBianjiangCore(root, deepLink = {}) {
  if (!root) return () => {};

  const cleanups = [];

  const nav = root.querySelectorAll('.bj-nav button');
  const activatePanel = (panelId) => {
    nav.forEach((b) => b.classList.toggle('is-active', b.dataset.panel === panelId));
    root.querySelectorAll('section.bj-panel').forEach((p) => {
  ...
  const stateEl = root.querySelector('#bj-storage-state');
  if (stateEl) {
  ...
  root.querySelectorAll('#bj-watch-list .bj-watch-item').forEach((item) => {
    const key = item.dataset.key;
    const btns = item.querySelectorAll('.bj-status-btn');
    const note = item.querySelector('.bj-watch-note');
    const stampEl = item.querySelector('.bj-watch-stamp');
    const rec = data[key] || {};
  ...
  activatePanel(bianjiangPanelId(deepLink.tab));

  return () => cleanups.forEach((fn) => fn());
}

export const initBianjiang = withGyInit(initBianjiangCore);
