/** 体制内人群 GY-07 · 页签 / 观测哨(命名空间 chinaos.tz.v1) */
import { AS_OF_BASELINE } from '../../lib/config/asOfBaseline.js';
import { tizhineiPanelId } from '../../lib/tizhinei/routing.js';
import { withGyInit } from '../shared/gy/enhanceMethodology.js';

const NS = 'chinaos.tz.v1';

/** @param {HTMLElement | null} root @param {{ tab?: string }} deepLink */
function initTizhineiCore(root, deepLink = {}) {
  if (!root) return () => {};

  const cleanups = [];

  const nav = root.querySelectorAll('.tz-nav button');
  const activatePanel = (panelId) => {
    nav.forEach((b) => b.classList.toggle('is-active', b.dataset.panel === panelId));
    root.querySelectorAll('section.tz-panel').forEach((p) => {
  ...
  const stateEl = root.querySelector('#tz-storage-state');
  if (stateEl) {
  ...
  root.querySelectorAll('#tz-watch-list .tz-watch-item').forEach((item) => {
    const key = item.dataset.key;
    const btns = item.querySelectorAll('.tz-status-btn');
    const note = item.querySelector('.tz-watch-note');
    const stampEl = item.querySelector('.tz-watch-stamp');
    const rec = data[key] || {};
  ...
  activatePanel(tizhineiPanelId(deepLink.tab));

  return () => cleanups.forEach((fn) => fn());
}

export const initTizhinei = withGyInit(initTizhineiCore);
