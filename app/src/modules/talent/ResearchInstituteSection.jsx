    ...
    <section>
      ...
      {!list.length ? (
        ...
        <>
          ...
          {view === 'stats' ? (
            <Grid cols={2}>
              <Card title="研究领域"><DistBars data={distField.slice(0, 12)} onPick={(k) => setField(field === k ? '' : k)} active={field} /></Card>
              <Card title={typeTab === RI_FACILITY_TYPE ? '运行状态' : '实验室层级'}><DistBars data={distTier} color={typeTab === RI_FACILITY_TYPE ? '#22c55e' : '#e8a317'} onPick={(k) => setTier(tier === k ? '' : k)} active={tier} /></Card>
            </Grid>
          ) : view === 'grid' ? (
            ...
            <div className="grid gap-4" style={{ gridTemplateColumns: '1.25fr 1fr' }}>
              <Card title={`${typeTab} (${filtered.length}/${tabList.length})`}>
