...
const ROLE_OPTS = ['', '总书记', '总理', '副总理', '国务委员', '人大委员长', '政协主席', '政协副主席', '政协秘书长', '纪委书记', '外交部长', '政法委书记', '中组部部长', '统战部部长', '中宣部部长', '人大副委员长', '人大秘书长', '监委主任', '军委副主席', '港澳办主任', '党委书记', '省长', '市长', '自治区主席', '部长', '国防部长', '署长', '局长', '最高法院长', '最高检检察长'];
const SECTOR_OPTS = ['', '国务院', '党中央', '国家机关', '全国政协', '国务院直属机构', '军队', '地方'];
...
export default function Page() {
  ...
  const filtered = useMemo(() => (figures || []).filter((f) => {
    const hay = [f.name, f.org, f.fields?.title, f.fields?.institution, f.fields?.rank, f.fields?.native, f.province, short(f.province), f.role, f.level, f.sector, f.raw, ...(f.career || []).map((c) => c.desc)].join(' ');
  ...
  }), [figures, q, prov, level, role, sector]);
  ...
  return (
    <div>
      ...
      <Grid cols={4} className="mb-6">
        ...
        <Stat value={secCount || '—'} label="省级书记" accent="#e8a317" />
      </Grid>

      {figures.length < 10 && (
        <Card title="一键载入省部级公开履历" className="mb-4">
          <p className="text-sm mb-3" style={{ color: 'var(--text-secondary)' }}>
            内置 {FIGURE_SEED.length} 条：省级 {FIGURE_CATALOG_META.breakdown?.provincial} + 中央部委 {FIGURE_CATALOG_META.breakdown?.central} + 人大政协/直属机构 {FIGURE_CATALOG_META.breakdown?.extended}。来源：{FIGURE_CATALOG_META.sources.join('、')}。
        ...
        <div className="grid gap-4" style={{ gridTemplateColumns: '1.1fr 1fr' }}>
          <Card title={`检索结果 (${filtered.length}/${figures.length})${asOf ? ` · 数据截至 ${asOf}` : ''}`}>
            <div className="flex gap-2 flex-wrap mb-3">
              ...
              <select value={sector} onChange={(e) => setSector(e.target.value)} style={inp}><option value="">全部系统</option>{[...SECTOR_OPTS.filter(Boolean), ...sectors.filter((s) => !SECTOR_OPTS.includes(s))].map((s) => <option key={s} value={s}>{s}</option>)}</select>
              ...
              {filtered.map((f) => (
                <button key={f.id} type="button" onClick={() => setSel(f)} className="w-full text-left p-3 rounded" style={{ background: detail?.id === f.id ? 'rgba(196,30,58,0.14)' : 'var(--bg-elevated)', border: `1px solid ${detail?.id === f.id ? 'var(--china-red)' : 'transparent'}`, cursor: 'pointer' }}>
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {f.sector && <span className="text-[10px] mono px-1.5 py-0.5 rounded" style={{ background: 'rgba(212,175,55,0.12)', color: '#d4af37' }}>{f.sector}</span>}
                  </div>
                  <div className="text-[11px] mt-0.5" style={{ color: 'var(--text-tertiary)' }}>{f.org ? `${f.org} · ` : ''}{f.fields?.title || ''}</div>
                </button>
              ))}
