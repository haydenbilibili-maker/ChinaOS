...
export function initNongmingong(root, deepLink = {}) {
  ...
  const nav = root.querySelectorAll('.nm-nav button');
  const activatePanel = (panelId) => {
    nav.forEach((b) => b.classList.toggle('is-active', b.dataset.panel === panelId));
    root.querySelectorAll('section.nm-panel').forEach((p) => {
  ...
  const stateEl = root.querySelector('#nm-storage-state');
  if (stateEl) {
  ...
  root.querySelectorAll('#nm-watch-list .nm-watch-item').forEach((item) => {
    const key = item.dataset.key;
    const btns = item.querySelectorAll('.nm-status-btn');
    const note = item.querySelector('.nm-watch-note');
    const stampEl = item.querySelector('.nm-watch-stamp');
    const rec = data[key] || {};
  ...
  activatePanel(nongmingongPanelId(deepLink.tab));
