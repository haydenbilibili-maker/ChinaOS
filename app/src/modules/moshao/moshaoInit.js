/** 基层治理末梢 · 网格员、辅警、社工、协管 GY-24 · 页签 / 观测哨(命名空间 chinaos.ms.v1) */
import { AS_OF_BASELINE } from '../../lib/config/asOfBaseline.js';
import { moshaoPanelId } from '../../lib/moshao/routing.js';
import { withGyInit } from '../shared/gy/enhanceMethodology.js';

const NS = 'chinaos.ms.v1';

/** @param {HTMLElement | null} root @param {{ tab?: string }} deepLink */
function initMoshaoCore(root, deepLink = {}) {
  if (!root) return () => {};

  const cleanups = [];

  const nav = root.querySelectorAll('.ms-nav button');
  const activatePanel = (panelId) => {
    nav.forEach((b) => b.classList.toggle('is-active', b.dataset.panel === panelId));
    root.querySelectorAll('section.ms-panel').forEach((p) => {
  ...
  const stateEl = root.querySelector('#ms-storage-state');
  if (stateEl) {
  ...
  root.querySelectorAll('#ms-watch-list .ms-watch-item').forEach((item) => {
    const key = item.dataset.key;
    const btns = item.querySelectorAll('.ms-status-btn');
    const note = item.querySelector('.ms-watch-note');
    const stampEl = item.querySelector('.ms-watch-stamp');
    const rec = data[key] || {};
  ...
  activatePanel(moshaoPanelId(deepLink.tab));

  return () => cleanups.forEach((fn) => fn());
}

export const initMoshao = withGyInit(initMoshaoCore);
