...
export function initYiyi(root, deepLink = {}) {
  ...
  const nav = root.querySelectorAll('.ym-nav button');
  const activatePanel = (panelId) => {
    nav.forEach((b) => b.classList.toggle('is-active', b.dataset.panel === panelId));
    root.querySelectorAll('section.ym-panel').forEach((p) => {
  ...
  const stateEl = root.querySelector('#ym-storage-state');
  if (stateEl) {
  ...
  root.querySelectorAll('#ym-watch-list .ym-watch-item').forEach((item) => {
    const key = item.dataset.key;
    const btns = item.querySelectorAll('.ym-status-btn');
    const note = item.querySelector('.ym-watch-note');
    const stampEl = item.querySelector('.ym-watch-stamp');
    const rec = data[key] || {};
  ...
  activatePanel(yiyiPanelId(deepLink.tab));
