/** 现役军人 GY-47 · 页签 / 观测哨(命名空间 chinaos.jr.v1) */
import { AS_OF_BASELINE } from '../../lib/config/asOfBaseline.js';
import { junrenPanelId } from '../../lib/junren/routing.js';
import { withGyInit } from '../shared/gy/enhanceMethodology.js';

const NS = 'chinaos.jr.v1';

/** @param {HTMLElement | null} root @param {{ tab?: string }} deepLink */
function initJunrenCore(root, deepLink = {}) {
  if (!root) return () => {};

  const cleanups = [];

  const nav = root.querySelectorAll('.jr-nav button');
  const activatePanel = (panelId) => {
    nav.forEach((b) => b.classList.toggle('is-active', b.dataset.panel === panelId));
    root.querySelectorAll('section.jr-panel').forEach((p) => {
  ...
  const stateEl = root.querySelector('#jr-storage-state');
  if (stateEl) {
  ...
  root.querySelectorAll('#jr-watch-list .jr-watch-item').forEach((item) => {
    const key = item.dataset.key;
    const btns = item.querySelectorAll('.jr-status-btn');
    const note = item.querySelector('.jr-watch-note');
    const stampEl = item.querySelector('.jr-watch-stamp');
    const rec = data[key] || {};
  ...
  activatePanel(junrenPanelId(deepLink.tab));

  return () => cleanups.forEach((fn) => fn());
}

export const initJunren = withGyInit(initJunrenCore);
