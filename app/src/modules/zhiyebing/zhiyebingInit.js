/** 尘肺与职业病/工伤群体 GY-50 · 页签 / 观测哨(命名空间 chinaos.zb.v1) */
import { AS_OF_BASELINE } from '../../lib/config/asOfBaseline.js';
import { zhiyebingPanelId } from '../../lib/zhiyebing/routing.js';
import { withGyInit } from '../shared/gy/enhanceMethodology.js';

const NS = 'chinaos.zb.v1';

/** @param {HTMLElement | null} root @param {{ tab?: string }} deepLink */
function initZhiyebingCore(root, deepLink = {}) {
  if (!root) return () => {};

  const cleanups = [];

  const nav = root.querySelectorAll('.zb-nav button');
  const activatePanel = (panelId) => {
    nav.forEach((b) => b.classList.toggle('is-active', b.dataset.panel === panelId));
    root.querySelectorAll('section.zb-panel').forEach((p) => {
  ...
  const stateEl = root.querySelector('#zb-storage-state');
  if (stateEl) {
  ...
  root.querySelectorAll('#zb-watch-list .zb-watch-item').forEach((item) => {
    const key = item.dataset.key;
    const btns = item.querySelectorAll('.zb-status-btn');
    const note = item.querySelector('.zb-watch-note');
    const stampEl = item.querySelector('.zb-watch-stamp');
    const rec = data[key] || {};
  ...
  activatePanel(zhiyebingPanelId(deepLink.tab));

  return () => cleanups.forEach((fn) => fn());
}

export const initZhiyebing = withGyInit(initZhiyebingCore);
