/** 精神障碍者与被监护人 GY-55 · 页签 / 观测哨(命名空间 chinaos.jh.v1) */
import { AS_OF_BASELINE } from '../../lib/config/asOfBaseline.js';
import { jingzhangPanelId } from '../../lib/jingzhang/routing.js';
import { withGyInit } from '../shared/gy/enhanceMethodology.js';

const NS = 'chinaos.jh.v1';

/** @param {HTMLElement | null} root @param {{ tab?: string }} deepLink */
function initJingzhangCore(root, deepLink = {}) {
  if (!root) return () => {};

  const cleanups = [];

  const nav = root.querySelectorAll('.jh-nav button');
  const activatePanel = (panelId) => {
    nav.forEach((b) => b.classList.toggle('is-active', b.dataset.panel === panelId));
    root.querySelectorAll('section.jh-panel').forEach((p) => {
  ...
  const stateEl = root.querySelector('#jh-storage-state');
  if (stateEl) {
  ...
  root.querySelectorAll('#jh-watch-list .jh-watch-item').forEach((item) => {
    const key = item.dataset.key;
    const btns = item.querySelectorAll('.jh-status-btn');
    const note = item.querySelector('.jh-watch-note');
    const stampEl = item.querySelector('.jh-watch-stamp');
    const rec = data[key] || {};
  ...
  activatePanel(jingzhangPanelId(deepLink.tab));

  return () => cleanups.forEach((fn) => fn());
}

export const initJingzhang = withGyInit(initJingzhangCore);
