/** 残障人群 · 可见性的零点 GY-18 · 页签 / 观测哨(命名空间 chinaos.cz.v1) */
import { AS_OF_BASELINE } from '../../lib/config/asOfBaseline.js';
import { canzhangPanelId } from '../../lib/canzhang/routing.js';
import { withGyInit } from '../shared/gy/enhanceMethodology.js';

const NS = 'chinaos.cz.v1';

/** @param {HTMLElement | null} root @param {{ tab?: string }} deepLink */
function initCanzhangCore(root, deepLink = {}) {
  if (!root) return () => {};

  const cleanups = [];

  const nav = root.querySelectorAll('.cz-nav button');
  const activatePanel = (panelId) => {
    nav.forEach((b) => b.classList.toggle('is-active', b.dataset.panel === panelId));
    root.querySelectorAll('section.cz-panel').forEach((p) => {
  ...
  const stateEl = root.querySelector('#cz-storage-state');
  if (stateEl) {
  ...
  root.querySelectorAll('#cz-watch-list .cz-watch-item').forEach((item) => {
    const key = item.dataset.key;
    const btns = item.querySelectorAll('.cz-status-btn');
    const note = item.querySelector('.cz-watch-note');
    const stampEl = item.querySelector('.cz-watch-stamp');
    const rec = data[key] || {};
  ...
  activatePanel(canzhangPanelId(deepLink.tab));

  return () => cleanups.forEach((fn) => fn());
}

export const initCanzhang = withGyInit(initCanzhangCore);
