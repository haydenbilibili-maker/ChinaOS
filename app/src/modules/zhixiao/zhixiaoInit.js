...
export function initZhixiao(root, deepLink = {}) {
  ...
  const nav = root.querySelectorAll('.zx-nav button');
  const activatePanel = (panelId) => {
    nav.forEach((b) => b.classList.toggle('is-active', b.dataset.panel === panelId));
    root.querySelectorAll('section.zx-panel').forEach((p) => {
  ...
  const stateEl = root.querySelector('#zx-storage-state');
  if (stateEl) {
  ...
  root.querySelectorAll('#zx-watch-list .zx-watch-item').forEach((item) => {
    const key = item.dataset.key;
    const btns = item.querySelectorAll('.zx-status-btn');
    const note = item.querySelector('.zx-watch-note');
    const stampEl = item.querySelector('.zx-watch-stamp');
    const rec = data[key] || {};
  ...
  activatePanel(zhixiaoPanelId(deepLink.tab));
