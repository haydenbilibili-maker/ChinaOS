/** 职业农民与新农人 GY-35 · 页签 / 观测哨(命名空间 chinaos.xn.v1) */
import { AS_OF_BASELINE } from '../../lib/config/asOfBaseline.js';
import { xinnongPanelId } from '../../lib/xinnong/routing.js';
import { withGyInit } from '../shared/gy/enhanceMethodology.js';

const NS = 'chinaos.xn.v1';

/** @param {HTMLElement | null} root @param {{ tab?: string }} deepLink */
function initXinnongCore(root, deepLink = {}) {
  if (!root) return () => {};

  const cleanups = [];

  const nav = root.querySelectorAll('.xn-nav button');
  const activatePanel = (panelId) => {
    nav.forEach((b) => b.classList.toggle('is-active', b.dataset.panel === panelId));
    root.querySelectorAll('section.xn-panel').forEach((p) => {
  ...
  const stateEl = root.querySelector('#xn-storage-state');
  if (stateEl) {
  ...
  root.querySelectorAll('#xn-watch-list .xn-watch-item').forEach((item) => {
    const key = item.dataset.key;
    const btns = item.querySelectorAll('.xn-status-btn');
    const note = item.querySelector('.xn-watch-note');
    const stampEl = item.querySelector('.xn-watch-stamp');
    const rec = data[key] || {};
  ...
  activatePanel(xinnongPanelId(deepLink.tab));

  return () => cleanups.forEach((fn) => fn());
}

export const initXinnong = withGyInit(initXinnongCore);
