/** 被拐卖与反拐救助对象 GY-52 · 页签 / 观测哨(命名空间 chinaos.fg.v1) */
import { AS_OF_BASELINE } from '../../lib/config/asOfBaseline.js';
import { fanguiPanelId } from '../../lib/fangui/routing.js';
import { withGyInit } from '../shared/gy/enhanceMethodology.js';

const NS = 'chinaos.fg.v1';

/** @param {HTMLElement | null} root @param {{ tab?: string }} deepLink */
function initFanguiCore(root, deepLink = {}) {
  if (!root) return () => {};

  const cleanups = [];

  const nav = root.querySelectorAll('.fg-nav button');
  const activatePanel = (panelId) => {
    nav.forEach((b) => b.classList.toggle('is-active', b.dataset.panel === panelId));
    root.querySelectorAll('section.fg-panel').forEach((p) => {
  ...
  const stateEl = root.querySelector('#fg-storage-state');
  if (stateEl) {
  ...
  root.querySelectorAll('#fg-watch-list .fg-watch-item').forEach((item) => {
    const key = item.dataset.key;
    const btns = item.querySelectorAll('.fg-status-btn');
    const note = item.querySelector('.fg-watch-note');
    const stampEl = item.querySelector('.fg-watch-stamp');
    const rec = data[key] || {};
  ...
  activatePanel(fanguiPanelId(deepLink.tab));

  return () => cleanups.forEach((fn) => fn());
}

export const initFangui = withGyInit(initFanguiCore);
