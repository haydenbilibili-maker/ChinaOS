/** 网约车司机 GY-40 · 页签 / 观测哨(命名空间 chinaos.wy.v1) */
import { AS_OF_BASELINE } from '../../lib/config/asOfBaseline.js';
import { wangyuePanelId } from '../../lib/wangyue/routing.js';
import { withGyInit } from '../shared/gy/enhanceMethodology.js';

const NS = 'chinaos.wy.v1';

/** @param {HTMLElement | null} root @param {{ tab?: string }} deepLink */
function initWangyueCore(root, deepLink = {}) {
  if (!root) return () => {};

  const cleanups = [];

  const nav = root.querySelectorAll('.wy-nav button');
  const activatePanel = (panelId) => {
    nav.forEach((b) => b.classList.toggle('is-active', b.dataset.panel === panelId));
    root.querySelectorAll('section.wy-panel').forEach((p) => {
  ...
  const stateEl = root.querySelector('#wy-storage-state');
  if (stateEl) {
  ...
  root.querySelectorAll('#wy-watch-list .wy-watch-item').forEach((item) => {
    const key = item.dataset.key;
    const btns = item.querySelectorAll('.wy-status-btn');
    const note = item.querySelector('.wy-watch-note');
    const stampEl = item.querySelector('.wy-watch-stamp');
    const rec = data[key] || {};
  ...
  activatePanel(wangyuePanelId(deepLink.tab));

  return () => cleanups.forEach((fn) => fn());
}

export const initWangyue = withGyInit(initWangyueCore);
