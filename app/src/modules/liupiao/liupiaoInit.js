...
export function initLiupiao(root, deepLink = {}) {
  ...
  const nav = root.querySelectorAll('.lp-nav button');
  const activatePanel = (panelId) => {
    nav.forEach((b) => b.classList.toggle('is-active', b.dataset.panel === panelId));
    root.querySelectorAll('section.lp-panel').forEach((p) => {
  ...
  const stateEl = root.querySelector('#lp-storage-state');
  if (stateEl) {
  ...
  root.querySelectorAll('#lp-watch-list .lp-watch-item').forEach((item) => {
    const key = item.dataset.key;
    const btns = item.querySelectorAll('.lp-status-btn');
    const note = item.querySelector('.lp-watch-note');
    const stampEl = item.querySelector('.lp-watch-stamp');
    const rec = data[key] || {};
  ...
  activatePanel(liupiaoPanelId(deepLink.tab));
