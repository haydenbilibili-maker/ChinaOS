/** 农村留守老人 GY-30 · 页签 / 观测哨(命名空间 chinaos.ll.v1) */
import { AS_OF_BASELINE } from '../../lib/config/asOfBaseline.js';
import { liushouPanelId } from '../../lib/liushou/routing.js';
import { withGyInit } from '../shared/gy/enhanceMethodology.js';

const NS = 'chinaos.ll.v1';

/** @param {HTMLElement | null} root @param {{ tab?: string }} deepLink */
function initLiushouCore(root, deepLink = {}) {
  if (!root) return () => {};

  const cleanups = [];

  const nav = root.querySelectorAll('.ll-nav button');
  const activatePanel = (panelId) => {
    nav.forEach((b) => b.classList.toggle('is-active', b.dataset.panel === panelId));
    root.querySelectorAll('section.ll-panel').forEach((p) => {
  ...
  const stateEl = root.querySelector('#ll-storage-state');
  if (stateEl) {
  ...
  root.querySelectorAll('#ll-watch-list .ll-watch-item').forEach((item) => {
    const key = item.dataset.key;
    const btns = item.querySelectorAll('.ll-status-btn');
    const note = item.querySelector('.ll-watch-note');
    const stampEl = item.querySelector('.ll-watch-stamp');
    const rec = data[key] || {};
  ...
  activatePanel(liushouPanelId(deepLink.tab));

  return () => cleanups.forEach((fn) => fn());
}

export const initLiushou = withGyInit(initLiushouCore);
