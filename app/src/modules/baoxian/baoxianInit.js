/** 保险代理与直销末梢 GY-41 · 页签 / 观测哨(命名空间 chinaos.bx.v1) */
import { AS_OF_BASELINE } from '../../lib/config/asOfBaseline.js';
import { baoxianPanelId } from '../../lib/baoxian/routing.js';
import { withGyInit } from '../shared/gy/enhanceMethodology.js';

const NS = 'chinaos.bx.v1';

/** @param {HTMLElement | null} root @param {{ tab?: string }} deepLink */
function initBaoxianCore(root, deepLink = {}) {
  if (!root) return () => {};

  const cleanups = [];

  const nav = root.querySelectorAll('.bx-nav button');
  const activatePanel = (panelId) => {
    nav.forEach((b) => b.classList.toggle('is-active', b.dataset.panel === panelId));
    root.querySelectorAll('section.bx-panel').forEach((p) => {
  ...
  const stateEl = root.querySelector('#bx-storage-state');
  if (stateEl) {
  ...
  root.querySelectorAll('#bx-watch-list .bx-watch-item').forEach((item) => {
    const key = item.dataset.key;
    const btns = item.querySelectorAll('.bx-status-btn');
    const note = item.querySelector('.bx-watch-note');
    const stampEl = item.querySelector('.bx-watch-stamp');
    const rec = data[key] || {};
  ...
  activatePanel(baoxianPanelId(deepLink.tab));

  return () => cleanups.forEach((fn) => fn());
}

export const initBaoxian = withGyInit(initBaoxianCore);
