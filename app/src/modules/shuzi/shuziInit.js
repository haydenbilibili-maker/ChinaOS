/** 数字原住民 · 10 后与屏幕养大的一代 GY-26 · 页签 / 观测哨(命名空间 chinaos.sz.v1) */
import { AS_OF_BASELINE } from '../../lib/config/asOfBaseline.js';
import { shuziPanelId } from '../../lib/shuzi/routing.js';
import { withGyInit } from '../shared/gy/enhanceMethodology.js';

const NS = 'chinaos.sz.v1';

/** @param {HTMLElement | null} root @param {{ tab?: string }} deepLink */
function initShuziCore(root, deepLink = {}) {
  if (!root) return () => {};

  const cleanups = [];

  const nav = root.querySelectorAll('.sz-nav button');
  const activatePanel = (panelId) => {
    nav.forEach((b) => b.classList.toggle('is-active', b.dataset.panel === panelId));
    root.querySelectorAll('section.sz-panel').forEach((p) => {
  ...
  const stateEl = root.querySelector('#sz-storage-state');
  if (stateEl) {
  ...
  root.querySelectorAll('#sz-watch-list .sz-watch-item').forEach((item) => {
    const key = item.dataset.key;
    const btns = item.querySelectorAll('.sz-status-btn');
    const note = item.querySelector('.sz-watch-note');
    const stampEl = item.querySelector('.sz-watch-stamp');
    const rec = data[key] || {};
  ...
  activatePanel(shuziPanelId(deepLink.tab));

  return () => cleanups.forEach((fn) => fn());
}

export const initShuzi = withGyInit(initShuziCore);
