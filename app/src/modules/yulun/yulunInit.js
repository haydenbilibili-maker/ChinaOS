/** 网络舆论场人群 GY-36 · 页签 / 观测哨(命名空间 chinaos.yl.v1) */
import { AS_OF_BASELINE } from '../../lib/config/asOfBaseline.js';
import { yulunPanelId } from '../../lib/yulun/routing.js';
import { withGyInit } from '../shared/gy/enhanceMethodology.js';

const NS = 'chinaos.yl.v1';

/** @param {HTMLElement | null} root @param {{ tab?: string }} deepLink */
function initYulunCore(root, deepLink = {}) {
  if (!root) return () => {};

  const cleanups = [];

  const nav = root.querySelectorAll('.yl-nav button');
  const activatePanel = (panelId) => {
    nav.forEach((b) => b.classList.toggle('is-active', b.dataset.panel === panelId));
    root.querySelectorAll('section.yl-panel').forEach((p) => {
  ...
  const stateEl = root.querySelector('#yl-storage-state');
  if (stateEl) {
  ...
  root.querySelectorAll('#yl-watch-list .yl-watch-item').forEach((item) => {
    const key = item.dataset.key;
    const btns = item.querySelectorAll('.yl-status-btn');
    const note = item.querySelector('.yl-watch-note');
    const stampEl = item.querySelector('.yl-watch-stamp');
    const rec = data[key] || {};
  ...
  activatePanel(yulunPanelId(deepLink.tab));

  return () => cleanups.forEach((fn) => fn());
}

export const initYulun = withGyInit(initYulunCore);
