/** 失信被执行人与债务人群 GY-32 · 页签 / 观测哨(命名空间 chinaos.zw.v1) */
import { AS_OF_BASELINE } from '../../lib/config/asOfBaseline.js';
import { zhaiwuPanelId } from '../../lib/zhaiwu/routing.js';
import { withGyInit } from '../shared/gy/enhanceMethodology.js';

const NS = 'chinaos.zw.v1';

/** @param {HTMLElement | null} root @param {{ tab?: string }} deepLink */
function initZhaiwuCore(root, deepLink = {}) {
  if (!root) return () => {};

  const cleanups = [];

  const nav = root.querySelectorAll('.zw-nav button');
  const activatePanel = (panelId) => {
    nav.forEach((b) => b.classList.toggle('is-active', b.dataset.panel === panelId));
    root.querySelectorAll('section.zw-panel').forEach((p) => {
  ...
  const stateEl = root.querySelector('#zw-storage-state');
  if (stateEl) {
  ...
  root.querySelectorAll('#zw-watch-list .zw-watch-item').forEach((item) => {
    const key = item.dataset.key;
    const btns = item.querySelectorAll('.zw-status-btn');
    const note = item.querySelector('.zw-watch-note');
    const stampEl = item.querySelector('.zw-watch-stamp');
    const rec = data[key] || {};
  ...
  activatePanel(zhaiwuPanelId(deepLink.tab));

  return () => cleanups.forEach((fn) => fn());
}

export const initZhaiwu = withGyInit(initZhaiwuCore);
