/** 县域青年 · 留下的人 GY-20 · 页签 / 观测哨(命名空间 chinaos.xy.v1) */
import { AS_OF_BASELINE } from '../../lib/config/asOfBaseline.js';
import { xianyuPanelId } from '../../lib/xianyu/routing.js';
import { withGyInit } from '../shared/gy/enhanceMethodology.js';

const NS = 'chinaos.xy.v1';

/** @param {HTMLElement | null} root @param {{ tab?: string }} deepLink */
function initXianyuCore(root, deepLink = {}) {
  if (!root) return () => {};

  const cleanups = [];

  const nav = root.querySelectorAll('.xy-nav button');
  const activatePanel = (panelId) => {
    nav.forEach((b) => b.classList.toggle('is-active', b.dataset.panel === panelId));
    root.querySelectorAll('section.xy-panel').forEach((p) => {
  ...
  const stateEl = root.querySelector('#xy-storage-state');
  if (stateEl) {
  ...
  root.querySelectorAll('#xy-watch-list .xy-watch-item').forEach((item) => {
    const key = item.dataset.key;
    const btns = item.querySelectorAll('.xy-status-btn');
    const note = item.querySelector('.xy-watch-note');
    const stampEl = item.querySelector('.xy-watch-stamp');
    const rec = data[key] || {};
  ...
  activatePanel(xianyuPanelId(deepLink.tab));

  return () => cleanups.forEach((fn) => fn());
}

export const initXianyu = withGyInit(initXianyuCore);
