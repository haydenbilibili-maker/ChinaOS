/** 工程移民与生态移民 GY-56 · 页签 / 观测哨(命名空间 chinaos.gc.v1) */
import { AS_OF_BASELINE } from '../../lib/config/asOfBaseline.js';
import { gongyiPanelId } from '../../lib/gongyi/routing.js';
import { withGyInit } from '../shared/gy/enhanceMethodology.js';

const NS = 'chinaos.gc.v1';

/** @param {HTMLElement | null} root @param {{ tab?: string }} deepLink */
function initGongyiCore(root, deepLink = {}) {
  if (!root) return () => {};

  const cleanups = [];

  const nav = root.querySelectorAll('.gc-nav button');
  const activatePanel = (panelId) => {
    nav.forEach((b) => b.classList.toggle('is-active', b.dataset.panel === panelId));
    root.querySelectorAll('section.gc-panel').forEach((p) => {
  ...
  const stateEl = root.querySelector('#gc-storage-state');
  if (stateEl) {
  ...
  root.querySelectorAll('#gc-watch-list .gc-watch-item').forEach((item) => {
    const key = item.dataset.key;
    const btns = item.querySelectorAll('.gc-status-btn');
    const note = item.querySelector('.gc-watch-note');
    const stampEl = item.querySelector('.gc-watch-stamp');
    const rec = data[key] || {};
  ...
  activatePanel(gongyiPanelId(deepLink.tab));

  return () => cleanups.forEach((fn) => fn());
}

export const initGongyi = withGyInit(initGongyiCore);
