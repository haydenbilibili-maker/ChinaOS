/** 受污名疾病与感染者群体 GY-57 · 页签 / 观测哨(命名空间 chinaos.gr.v1) */
import { AS_OF_BASELINE } from '../../lib/config/asOfBaseline.js';
import { ganranPanelId } from '../../lib/ganran/routing.js';
import { withGyInit } from '../shared/gy/enhanceMethodology.js';

const NS = 'chinaos.gr.v1';

/** @param {HTMLElement | null} root @param {{ tab?: string }} deepLink */
function initGanranCore(root, deepLink = {}) {
  if (!root) return () => {};

  const cleanups = [];

  const nav = root.querySelectorAll('.gr-nav button');
  const activatePanel = (panelId) => {
    nav.forEach((b) => b.classList.toggle('is-active', b.dataset.panel === panelId));
    root.querySelectorAll('section.gr-panel').forEach((p) => {
  ...
  const stateEl = root.querySelector('#gr-storage-state');
  if (stateEl) {
  ...
  root.querySelectorAll('#gr-watch-list .gr-watch-item').forEach((item) => {
    const key = item.dataset.key;
    const btns = item.querySelectorAll('.gr-status-btn');
    const note = item.querySelector('.gr-watch-note');
    const stampEl = item.querySelector('.gr-watch-stamp');
    const rec = data[key] || {};
  ...
  activatePanel(ganranPanelId(deepLink.tab));

  return () => cleanups.forEach((fn) => fn());
}

export const initGanran = withGyInit(initGanranCore);
