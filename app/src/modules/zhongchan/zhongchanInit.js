...
function initZhongchanCore(root, deepLink = {}) {
  ...
  const nav = root.querySelectorAll('.zc-nav button');
  const activatePanel = (panelId) => {
    nav.forEach((b) => b.classList.toggle('is-active', b.dataset.panel === panelId));
    root.querySelectorAll('section.zc-panel').forEach((p) => {
  ...
  const stateEl = root.querySelector('#zc-storage-state');
  if (stateEl) {
  ...
  root.querySelectorAll('#zc-watch-list .zc-watch-item').forEach((item) => {
    const key = item.dataset.key;
    const btns = item.querySelectorAll('.zc-status-btn');
    const note = item.querySelector('.zc-watch-note');
    const stampEl = item.querySelector('.zc-watch-stamp');
    const rec = data[key] || {};
  ...
  activatePanel(zhongchanPanelId(deepLink.tab));
