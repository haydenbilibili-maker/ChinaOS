/** 带病生存的年轻人 · 健康的阶层化 GY-23 · 页签 / 观测哨(命名空间 chinaos.mb.v1) */
import { AS_OF_BASELINE } from '../../lib/config/asOfBaseline.js';
import { manbingPanelId } from '../../lib/manbing/routing.js';
import { withGyInit } from '../shared/gy/enhanceMethodology.js';

const NS = 'chinaos.mb.v1';

/** @param {HTMLElement | null} root @param {{ tab?: string }} deepLink */
function initManbingCore(root, deepLink = {}) {
  if (!root) return () => {};

  const cleanups = [];

  const nav = root.querySelectorAll('.mb-nav button');
  const activatePanel = (panelId) => {
    nav.forEach((b) => b.classList.toggle('is-active', b.dataset.panel === panelId));
    root.querySelectorAll('section.mb-panel').forEach((p) => {
  ...
  const stateEl = root.querySelector('#mb-storage-state');
  if (stateEl) {
  ...
  root.querySelectorAll('#mb-watch-list .mb-watch-item').forEach((item) => {
    const key = item.dataset.key;
    const btns = item.querySelectorAll('.mb-status-btn');
    const note = item.querySelector('.mb-watch-note');
    const stampEl = item.querySelector('.mb-watch-stamp');
    const rec = data[key] || {};
  ...
  activatePanel(manbingPanelId(deepLink.tab));

  return () => cleanups.forEach((fn) => fn());
}

export const initManbing = withGyInit(initManbingCore);
