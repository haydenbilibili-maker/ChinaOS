/** 中小民营企业主 GY-42 · 页签 / 观测哨(命名空间 chinaos.mq.v1) */
import { AS_OF_BASELINE } from '../../lib/config/asOfBaseline.js';
import { minqiPanelId } from '../../lib/minqi/routing.js';
import { withGyInit } from '../shared/gy/enhanceMethodology.js';

const NS = 'chinaos.mq.v1';

/** @param {HTMLElement | null} root @param {{ tab?: string }} deepLink */
function initMinqiCore(root, deepLink = {}) {
  if (!root) return () => {};

  const cleanups = [];

  const nav = root.querySelectorAll('.mq-nav button');
  const activatePanel = (panelId) => {
    nav.forEach((b) => b.classList.toggle('is-active', b.dataset.panel === panelId));
    root.querySelectorAll('section.mq-panel').forEach((p) => {
  ...
  const stateEl = root.querySelector('#mq-storage-state');
  if (stateEl) {
  ...
  root.querySelectorAll('#mq-watch-list .mq-watch-item').forEach((item) => {
    const key = item.dataset.key;
    const btns = item.querySelectorAll('.mq-status-btn');
    const note = item.querySelector('.mq-watch-note');
    const stampEl = item.querySelector('.mq-watch-stamp');
    const rec = data[key] || {};
  ...
  activatePanel(minqiPanelId(deepLink.tab));

  return () => cleanups.forEach((fn) => fn());
}

export const initMinqi = withGyInit(initMinqiCore);
