/** 城市极贫与救助对象 GY-43 · 页签 / 观测哨(命名空间 chinaos.jp.v1) */
import { AS_OF_BASELINE } from '../../lib/config/asOfBaseline.js';
import { jipinPanelId } from '../../lib/jipin/routing.js';
import { withGyInit } from '../shared/gy/enhanceMethodology.js';

const NS = 'chinaos.jp.v1';

/** @param {HTMLElement | null} root @param {{ tab?: string }} deepLink */
function initJipinCore(root, deepLink = {}) {
  if (!root) return () => {};

  const cleanups = [];

  const nav = root.querySelectorAll('.jp-nav button');
  const activatePanel = (panelId) => {
    nav.forEach((b) => b.classList.toggle('is-active', b.dataset.panel === panelId));
    root.querySelectorAll('section.jp-panel').forEach((p) => {
  ...
  const stateEl = root.querySelector('#jp-storage-state');
  if (stateEl) {
  ...
  root.querySelectorAll('#jp-watch-list .jp-watch-item').forEach((item) => {
    const key = item.dataset.key;
    const btns = item.querySelectorAll('.jp-status-btn');
    const note = item.querySelector('.jp-watch-note');
    const stampEl = item.querySelector('.jp-watch-stamp');
    const rec = data[key] || {};
  ...
  activatePanel(jipinPanelId(deepLink.tab));

  return () => cleanups.forEach((fn) => fn());
}

export const initJipin = withGyInit(initJipinCore);
