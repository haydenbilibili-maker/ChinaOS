/** 保安群体 GY-39 · 页签 / 观测哨(命名空间 chinaos.ba.v1) */
import { AS_OF_BASELINE } from '../../lib/config/asOfBaseline.js';
import { baoanPanelId } from '../../lib/baoan/routing.js';
import { withGyInit } from '../shared/gy/enhanceMethodology.js';

const NS = 'chinaos.ba.v1';

/** @param {HTMLElement | null} root @param {{ tab?: string }} deepLink */
function initBaoanCore(root, deepLink = {}) {
  if (!root) return () => {};

  const cleanups = [];

  const nav = root.querySelectorAll('.ba-nav button');
  const activatePanel = (panelId) => {
    nav.forEach((b) => b.classList.toggle('is-active', b.dataset.panel === panelId));
    root.querySelectorAll('section.ba-panel').forEach((p) => {
  ...
  const stateEl = root.querySelector('#ba-storage-state');
  if (stateEl) {
  ...
  root.querySelectorAll('#ba-watch-list .ba-watch-item').forEach((item) => {
    const key = item.dataset.key;
    const btns = item.querySelectorAll('.ba-status-btn');
    const note = item.querySelector('.ba-watch-note');
    const stampEl = item.querySelector('.ba-watch-stamp');
    const rec = data[key] || {};
  ...
  activatePanel(baoanPanelId(deepLink.tab));

  return () => cleanups.forEach((fn) => fn());
}

export const initBaoan = withGyInit(initBaoanCore);
