...
export function initLaonian(root, deepLink = {}) {
  ...
  const nav = root.querySelectorAll('.ln-nav button');
  const activatePanel = (panelId) => {
    nav.forEach((b) => b.classList.toggle('is-active', b.dataset.panel === panelId));
    root.querySelectorAll('section.ln-panel').forEach((p) => {
  ...
  const stateEl = root.querySelector('#ln-storage-state');
  if (stateEl) {
  ...
  root.querySelectorAll('#ln-watch-list .ln-watch-item').forEach((item) => {
    const key = item.dataset.key;
    const btns = item.querySelectorAll('.ln-status-btn');
    const note = item.querySelector('.ln-watch-note');
    const stampEl = item.querySelector('.ln-watch-stamp');
    const rec = data[key] || {};
  ...
  activatePanel(laonianPanelId(deepLink.tab));
