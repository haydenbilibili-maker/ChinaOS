/** 农村留守妇女 GY-45 · 页签 / 观测哨(命名空间 chinaos.fn.v1) */
import { AS_OF_BASELINE } from '../../lib/config/asOfBaseline.js';
import { funvPanelId } from '../../lib/funv/routing.js';
import { withGyInit } from '../shared/gy/enhanceMethodology.js';

const NS = 'chinaos.fn.v1';

/** @param {HTMLElement | null} root @param {{ tab?: string }} deepLink */
function initFunvCore(root, deepLink = {}) {
  if (!root) return () => {};

  const cleanups = [];

  const nav = root.querySelectorAll('.fn-nav button');
  const activatePanel = (panelId) => {
    nav.forEach((b) => b.classList.toggle('is-active', b.dataset.panel === panelId));
    root.querySelectorAll('section.fn-panel').forEach((p) => {
  ...
  const stateEl = root.querySelector('#fn-storage-state');
  if (stateEl) {
  ...
  root.querySelectorAll('#fn-watch-list .fn-watch-item').forEach((item) => {
    const key = item.dataset.key;
    const btns = item.querySelectorAll('.fn-status-btn');
    const note = item.querySelector('.fn-watch-note');
    const stampEl = item.querySelector('.fn-watch-stamp');
    const rec = data[key] || {};
  ...
  activatePanel(funvPanelId(deepLink.tab));

  return () => cleanups.forEach((fn) => fn());
}

export const initFunv = withGyInit(initFunvCore);
