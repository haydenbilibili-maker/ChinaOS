/** 戒毒与社区康复人员 GY-49 · 页签 / 观测哨(命名空间 chinaos.jd.v1) */
import { AS_OF_BASELINE } from '../../lib/config/asOfBaseline.js';
import { jieduPanelId } from '../../lib/jiedu/routing.js';
import { withGyInit } from '../shared/gy/enhanceMethodology.js';

const NS = 'chinaos.jd.v1';

/** @param {HTMLElement | null} root @param {{ tab?: string }} deepLink */
function initJieduCore(root, deepLink = {}) {
  if (!root) return () => {};

  const cleanups = [];

  const nav = root.querySelectorAll('.jd-nav button');
  const activatePanel = (panelId) => {
    nav.forEach((b) => b.classList.toggle('is-active', b.dataset.panel === panelId));
    root.querySelectorAll('section.jd-panel').forEach((p) => {
  ...
  const stateEl = root.querySelector('#jd-storage-state');
  if (stateEl) {
  ...
  root.querySelectorAll('#jd-watch-list .jd-watch-item').forEach((item) => {
    const key = item.dataset.key;
    const btns = item.querySelectorAll('.jd-status-btn');
    const note = item.querySelector('.jd-watch-note');
    const stampEl = item.querySelector('.jd-watch-stamp');
    const rec = data[key] || {};
  ...
  activatePanel(jieduPanelId(deepLink.tab));

  return () => cleanups.forEach((fn) => fn());
}

export const initJiedu = withGyInit(initJieduCore);
