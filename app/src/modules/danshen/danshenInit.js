/** 单身女性与不婚者 · 主动退出的常态化 GY-19 · 页签 / 观测哨(命名空间 chinaos.ds.v1) */
import { AS_OF_BASELINE } from '../../lib/config/asOfBaseline.js';
import { danshenPanelId } from '../../lib/danshen/routing.js';
import { withGyInit } from '../shared/gy/enhanceMethodology.js';

const NS = 'chinaos.ds.v1';

/** @param {HTMLElement | null} root @param {{ tab?: string }} deepLink */
function initDanshenCore(root, deepLink = {}) {
  if (!root) return () => {};

  const cleanups = [];

  const nav = root.querySelectorAll('.ds-nav button');
  const activatePanel = (panelId) => {
    nav.forEach((b) => b.classList.toggle('is-active', b.dataset.panel === panelId));
    root.querySelectorAll('section.ds-panel').forEach((p) => {
  ...
  const stateEl = root.querySelector('#ds-storage-state');
  if (stateEl) {
  ...
  root.querySelectorAll('#ds-watch-list .ds-watch-item').forEach((item) => {
    const key = item.dataset.key;
    const btns = item.querySelectorAll('.ds-status-btn');
    const note = item.querySelector('.ds-watch-note');
    const stampEl = item.querySelector('.ds-watch-stamp');
    const rec = data[key] || {};
  ...
  activatePanel(danshenPanelId(deepLink.tab));

  return () => cleanups.forEach((fn) => fn());
}

export const initDanshen = withGyInit(initDanshenCore);
