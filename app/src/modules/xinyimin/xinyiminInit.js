/** 城市新移民与夹心层 · 有城无籍 GY-21 · 页签 / 观测哨(命名空间 chinaos.xm.v1) */
import { AS_OF_BASELINE } from '../../lib/config/asOfBaseline.js';
import { xinyiminPanelId } from '../../lib/xinyimin/routing.js';
import { withGyInit } from '../shared/gy/enhanceMethodology.js';

const NS = 'chinaos.xm.v1';

/** @param {HTMLElement | null} root @param {{ tab?: string }} deepLink */
function initXinyiminCore(root, deepLink = {}) {
  if (!root) return () => {};

  const cleanups = [];

  const nav = root.querySelectorAll('.xm-nav button');
  const activatePanel = (panelId) => {
    nav.forEach((b) => b.classList.toggle('is-active', b.dataset.panel === panelId));
    root.querySelectorAll('section.xm-panel').forEach((p) => {
  ...
  const stateEl = root.querySelector('#xm-storage-state');
  if (stateEl) {
  ...
  root.querySelectorAll('#xm-watch-list .xm-watch-item').forEach((item) => {
    const key = item.dataset.key;
    const btns = item.querySelectorAll('.xm-status-btn');
    const note = item.querySelector('.xm-watch-note');
    const stampEl = item.querySelector('.xm-watch-stamp');
    const rec = data[key] || {};
  ...
  activatePanel(xinyiminPanelId(deepLink.tab));

  return () => cleanups.forEach((fn) => fn());
}

export const initXinyimin = withGyInit(initXinyiminCore);
