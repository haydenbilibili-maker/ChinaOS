/** 制造业技术工人 · 被需要却不被向往 GY-22 · 页签 / 观测哨(命名空间 chinaos.jg.v1) */
import { AS_OF_BASELINE } from '../../lib/config/asOfBaseline.js';
import { jigongPanelId } from '../../lib/jigong/routing.js';
import { withGyInit } from '../shared/gy/enhanceMethodology.js';

const NS = 'chinaos.jg.v1';

/** @param {HTMLElement | null} root @param {{ tab?: string }} deepLink */
function initJigongCore(root, deepLink = {}) {
  if (!root) return () => {};

  const cleanups = [];

  const nav = root.querySelectorAll('.jg-nav button');
  const activatePanel = (panelId) => {
    nav.forEach((b) => b.classList.toggle('is-active', b.dataset.panel === panelId));
    root.querySelectorAll('section.jg-panel').forEach((p) => {
  ...
  const stateEl = root.querySelector('#jg-storage-state');
  if (stateEl) {
  ...
  root.querySelectorAll('#jg-watch-list .jg-watch-item').forEach((item) => {
    const key = item.dataset.key;
    const btns = item.querySelectorAll('.jg-status-btn');
    const note = item.querySelector('.jg-watch-note');
    const stampEl = item.querySelector('.jg-watch-stamp');
    const rec = data[key] || {};
  ...
  activatePanel(jigongPanelId(deepLink.tab));

  return () => cleanups.forEach((fn) => fn());
}

export const initJigong = withGyInit(initJigongCore);
