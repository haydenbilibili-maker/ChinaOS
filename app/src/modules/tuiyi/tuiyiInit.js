...
export function initTuiyi(root, deepLink = {}) {
  ...
  const nav = root.querySelectorAll('.ty-nav button');
  const activatePanel = (panelId) => {
    nav.forEach((b) => b.classList.toggle('is-active', b.dataset.panel === panelId));
    root.querySelectorAll('section.ty-panel').forEach((p) => {
  ...
  const stateEl = root.querySelector('#ty-storage-state');
  if (stateEl) {
  ...
  root.querySelectorAll('#ty-watch-list .ty-watch-item').forEach((item) => {
    const key = item.dataset.key;
    const btns = item.querySelectorAll('.ty-status-btn');
    const note = item.querySelector('.ty-watch-note');
    const stampEl = item.querySelector('.ty-watch-stamp');
    const rec = data[key] || {};
  ...
  activatePanel(tuiyiPanelId(deepLink.tab));
