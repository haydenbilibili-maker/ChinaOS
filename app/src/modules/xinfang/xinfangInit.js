/** 信访群体 GY-58 · 页签 / 观测哨(命名空间 chinaos.xf.v1) */
import { AS_OF_BASELINE } from '../../lib/config/asOfBaseline.js';
import { xinfangPanelId } from '../../lib/xinfang/routing.js';
import { withGyInit } from '../shared/gy/enhanceMethodology.js';

const NS = 'chinaos.xf.v1';

/** @param {HTMLElement | null} root @param {{ tab?: string }} deepLink */
function initXinfangCore(root, deepLink = {}) {
  if (!root) return () => {};

  const cleanups = [];

  const nav = root.querySelectorAll('.xf-nav button');
  const activatePanel = (panelId) => {
    nav.forEach((b) => b.classList.toggle('is-active', b.dataset.panel === panelId));
    root.querySelectorAll('section.xf-panel').forEach((p) => {
  ...
  const stateEl = root.querySelector('#xf-storage-state');
  if (stateEl) {
  ...
  root.querySelectorAll('#xf-watch-list .xf-watch-item').forEach((item) => {
    const key = item.dataset.key;
    const btns = item.querySelectorAll('.xf-status-btn');
    const note = item.querySelector('.xf-watch-note');
    const stampEl = item.querySelector('.xf-watch-stamp');
    const rec = data[key] || {};
  ...
  activatePanel(xinfangPanelId(deepLink.tab));

  return () => cleanups.forEach((fn) => fn());
}

export const initXinfang = withGyInit(initXinfangCore);
