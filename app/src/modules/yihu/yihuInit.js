/** 医护人员 GY-37 · 页签 / 观测哨(命名空间 chinaos.yh.v1) */
import { AS_OF_BASELINE } from '../../lib/config/asOfBaseline.js';
import { yihuPanelId } from '../../lib/yihu/routing.js';
import { withGyInit } from '../shared/gy/enhanceMethodology.js';

const NS = 'chinaos.yh.v1';

/** @param {HTMLElement | null} root @param {{ tab?: string }} deepLink */
function initYihuCore(root, deepLink = {}) {
  if (!root) return () => {};

  const cleanups = [];

  const nav = root.querySelectorAll('.yh-nav button');
  const activatePanel = (panelId) => {
    nav.forEach((b) => b.classList.toggle('is-active', b.dataset.panel === panelId));
    root.querySelectorAll('section.yh-panel').forEach((p) => {
  ...
  const stateEl = root.querySelector('#yh-storage-state');
  if (stateEl) {
  ...
  root.querySelectorAll('#yh-watch-list .yh-watch-item').forEach((item) => {
    const key = item.dataset.key;
    const btns = item.querySelectorAll('.yh-status-btn');
    const note = item.querySelector('.yh-watch-note');
    const stampEl = item.querySelector('.yh-watch-stamp');
    const rec = data[key] || {};
  ...
  activatePanel(yihuPanelId(deepLink.tab));

  return () => cleanups.forEach((fn) => fn());
}

export const initYihu = withGyInit(initYihuCore);
