/** 殡葬与临终关怀从业者 GY-46 · 页签 / 观测哨(命名空间 chinaos.bz.v1) */
import { AS_OF_BASELINE } from '../../lib/config/asOfBaseline.js';
import { binzangPanelId } from '../../lib/binzang/routing.js';
import { withGyInit } from '../shared/gy/enhanceMethodology.js';

const NS = 'chinaos.bz.v1';

/** @param {HTMLElement | null} root @param {{ tab?: string }} deepLink */
function initBinzangCore(root, deepLink = {}) {
  if (!root) return () => {};

  const cleanups = [];

  const nav = root.querySelectorAll('.bz-nav button');
  const activatePanel = (panelId) => {
    nav.forEach((b) => b.classList.toggle('is-active', b.dataset.panel === panelId));
    root.querySelectorAll('section.bz-panel').forEach((p) => {
  ...
  const stateEl = root.querySelector('#bz-storage-state');
  if (stateEl) {
  ...
  root.querySelectorAll('#bz-watch-list .bz-watch-item').forEach((item) => {
    const key = item.dataset.key;
    const btns = item.querySelectorAll('.bz-status-btn');
    const note = item.querySelector('.bz-watch-note');
    const stampEl = item.querySelector('.bz-watch-stamp');
    const rec = data[key] || {};
  ...
  activatePanel(binzangPanelId(deepLink.tab));

  return () => cleanups.forEach((fn) => fn());
}

export const initBinzang = withGyInit(initBinzangCore);
