/** 塔尖 · 高净值与企业家 GY-10 · 页签 / 观测哨(命名空间 chinaos.tj.v1) */
import { AS_OF_BASELINE } from '../../lib/config/asOfBaseline.js';
import { tajianPanelId } from '../../lib/tajian/routing.js';
import { withGyInit } from '../shared/gy/enhanceMethodology.js';

const NS = 'chinaos.tj.v1';

/** @param {HTMLElement | null} root @param {{ tab?: string }} deepLink */
function initTajianCore(root, deepLink = {}) {
  if (!root) return () => {};

  const cleanups = [];

  const nav = root.querySelectorAll('.tj-nav button');
  const activatePanel = (panelId) => {
    nav.forEach((b) => b.classList.toggle('is-active', b.dataset.panel === panelId));
    root.querySelectorAll('section.tj-panel').forEach((p) => {
  ...
  const stateEl = root.querySelector('#tj-storage-state');
  if (stateEl) {
  ...
  root.querySelectorAll('#tj-watch-list .tj-watch-item').forEach((item) => {
    const key = item.dataset.key;
    const btns = item.querySelectorAll('.tj-status-btn');
    const note = item.querySelector('.tj-watch-note');
    const stampEl = item.querySelector('.tj-watch-stamp');
    const rec = data[key] || {};
  ...
  activatePanel(tajianPanelId(deepLink.tab));

  return () => cleanups.forEach((fn) => fn());
}

export const initTajian = withGyInit(initTajianCore);
