/** 医保里的人 · 慢病与老龄财政 GY-15 · 页签 / 观测哨(命名空间 chinaos.yb.v1) */
import { AS_OF_BASELINE } from '../../lib/config/asOfBaseline.js';
import { yibaoPanelId } from '../../lib/yibao/routing.js';
import { withGyInit } from '../shared/gy/enhanceMethodology.js';

const NS = 'chinaos.yb.v1';

/** @param {HTMLElement | null} root @param {{ tab?: string }} deepLink */
function initYibaoCore(root, deepLink = {}) {
  if (!root) return () => {};

  const cleanups = [];

  const nav = root.querySelectorAll('.yb-nav button');
  const activatePanel = (panelId) => {
    nav.forEach((b) => b.classList.toggle('is-active', b.dataset.panel === panelId));
    root.querySelectorAll('section.yb-panel').forEach((p) => {
  ...
  const stateEl = root.querySelector('#yb-storage-state');
  if (stateEl) {
  ...
  root.querySelectorAll('#yb-watch-list .yb-watch-item').forEach((item) => {
    const key = item.dataset.key;
    const btns = item.querySelectorAll('.yb-status-btn');
    const note = item.querySelector('.yb-watch-note');
    const stampEl = item.querySelector('.yb-watch-stamp');
    const rec = data[key] || {};
  ...
  activatePanel(yibaoPanelId(deepLink.tab));

  return () => cleanups.forEach((fn) => fn());
}

export const initYibao = withGyInit(initYibaoCore);
