/** 失独与计生后遗人群 · 政策账单的活体 GY-25 · 页签 / 观测哨(命名空间 chinaos.sd.v1) */
import { AS_OF_BASELINE } from '../../lib/config/asOfBaseline.js';
import { shiduPanelId } from '../../lib/shidu/routing.js';
import { withGyInit } from '../shared/gy/enhanceMethodology.js';

const NS = 'chinaos.sd.v1';

/** @param {HTMLElement | null} root @param {{ tab?: string }} deepLink */
function initShiduCore(root, deepLink = {}) {
  if (!root) return () => {};

  const cleanups = [];

  const nav = root.querySelectorAll('.sd-nav button');
  const activatePanel = (panelId) => {
    nav.forEach((b) => b.classList.toggle('is-active', b.dataset.panel === panelId));
    root.querySelectorAll('section.sd-panel').forEach((p) => {
  ...
  const stateEl = root.querySelector('#sd-storage-state');
  if (stateEl) {
  ...
  root.querySelectorAll('#sd-watch-list .sd-watch-item').forEach((item) => {
    const key = item.dataset.key;
    const btns = item.querySelectorAll('.sd-status-btn');
    const note = item.querySelector('.sd-watch-note');
    const stampEl = item.querySelector('.sd-watch-stamp');
    const rec = data[key] || {};
  ...
  activatePanel(shiduPanelId(deepLink.tab));

  return () => cleanups.forEach((fn) => fn());
}

export const initShidu = withGyInit(initShiduCore);
