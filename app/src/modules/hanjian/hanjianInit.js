/** 罕见病与大病自救群体 GY-53 · 页签 / 观测哨(命名空间 chinaos.hj.v1) */
import { AS_OF_BASELINE } from '../../lib/config/asOfBaseline.js';
import { hanjianPanelId } from '../../lib/hanjian/routing.js';
import { withGyInit } from '../shared/gy/enhanceMethodology.js';

const NS = 'chinaos.hj.v1';

/** @param {HTMLElement | null} root @param {{ tab?: string }} deepLink */
function initHanjianCore(root, deepLink = {}) {
  if (!root) return () => {};

  const cleanups = [];

  const nav = root.querySelectorAll('.hj-nav button');
  const activatePanel = (panelId) => {
    nav.forEach((b) => b.classList.toggle('is-active', b.dataset.panel === panelId));
    root.querySelectorAll('section.hj-panel').forEach((p) => {
  ...
  const stateEl = root.querySelector('#hj-storage-state');
  if (stateEl) {
  ...
  root.querySelectorAll('#hj-watch-list .hj-watch-item').forEach((item) => {
    const key = item.dataset.key;
    const btns = item.querySelectorAll('.hj-status-btn');
    const note = item.querySelector('.hj-watch-note');
    const stampEl = item.querySelector('.hj-watch-stamp');
    const rec = data[key] || {};
  ...
  activatePanel(hanjianPanelId(deepLink.tab));

  return () => cleanups.forEach((fn) => fn());
}

export const initHanjian = withGyInit(initHanjianCore);
