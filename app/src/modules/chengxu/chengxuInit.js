/** 程序员与大厂白领 GY-28 · 页签 / 观测哨(命名空间 chinaos.cx.v1) */
import { AS_OF_BASELINE } from '../../lib/config/asOfBaseline.js';
import { chengxuPanelId } from '../../lib/chengxu/routing.js';
import { withGyInit } from '../shared/gy/enhanceMethodology.js';

const NS = 'chinaos.cx.v1';

/** @param {HTMLElement | null} root @param {{ tab?: string }} deepLink */
function initChengxuCore(root, deepLink = {}) {
  if (!root) return () => {};

  const cleanups = [];

  const nav = root.querySelectorAll('.cx-nav button');
  const activatePanel = (panelId) => {
    nav.forEach((b) => b.classList.toggle('is-active', b.dataset.panel === panelId));
    root.querySelectorAll('section.cx-panel').forEach((p) => {
  ...
  const stateEl = root.querySelector('#cx-storage-state');
  if (stateEl) {
  ...
  root.querySelectorAll('#cx-watch-list .cx-watch-item').forEach((item) => {
    const key = item.dataset.key;
    const btns = item.querySelectorAll('.cx-status-btn');
    const note = item.querySelector('.cx-watch-note');
    const stampEl = item.querySelector('.cx-watch-stamp');
    const rec = data[key] || {};
  ...
  activatePanel(chengxuPanelId(deepLink.tab));

  return () => cleanups.forEach((fn) => fn());
}

export const initChengxu = withGyInit(initChengxuCore);
