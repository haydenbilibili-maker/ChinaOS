/** 中年女性 · 被折叠的一代 GY-17 · 页签 / 观测哨(命名空间 chinaos.zn.v1) */
import { AS_OF_BASELINE } from '../../lib/config/asOfBaseline.js';
import { zhongnvPanelId } from '../../lib/zhongnv/routing.js';
import { withGyInit } from '../shared/gy/enhanceMethodology.js';

const NS = 'chinaos.zn.v1';

/** @param {HTMLElement | null} root @param {{ tab?: string }} deepLink */
function initZhongnvCore(root, deepLink = {}) {
  if (!root) return () => {};

  const cleanups = [];

  const nav = root.querySelectorAll('.zn-nav button');
  const activatePanel = (panelId) => {
    nav.forEach((b) => b.classList.toggle('is-active', b.dataset.panel === panelId));
    root.querySelectorAll('section.zn-panel').forEach((p) => {
  ...
  const stateEl = root.querySelector('#zn-storage-state');
  if (stateEl) {
  ...
  root.querySelectorAll('#zn-watch-list .zn-watch-item').forEach((item) => {
    const key = item.dataset.key;
    const btns = item.querySelectorAll('.zn-status-btn');
    const note = item.querySelector('.zn-watch-note');
    const stampEl = item.querySelector('.zn-watch-stamp');
    const rec = data[key] || {};
  ...
  activatePanel(zhongnvPanelId(deepLink.tab));

  return () => cleanups.forEach((fn) => fn());
}

export const initZhongnv = withGyInit(initZhongnvCore);
