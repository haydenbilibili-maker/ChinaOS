/** 个体工商户与小微商家 GY-29 · 页签 / 观测哨(命名空间 chinaos.gt.v1) */
import { AS_OF_BASELINE } from '../../lib/config/asOfBaseline.js';
import { getihuPanelId } from '../../lib/getihu/routing.js';
import { withGyInit } from '../shared/gy/enhanceMethodology.js';

const NS = 'chinaos.gt.v1';

/** @param {HTMLElement | null} root @param {{ tab?: string }} deepLink */
function initGetihuCore(root, deepLink = {}) {
  if (!root) return () => {};

  const cleanups = [];

  const nav = root.querySelectorAll('.gt-nav button');
  const activatePanel = (panelId) => {
    nav.forEach((b) => b.classList.toggle('is-active', b.dataset.panel === panelId));
    root.querySelectorAll('section.gt-panel').forEach((p) => {
  ...
  const stateEl = root.querySelector('#gt-storage-state');
  if (stateEl) {
  ...
  root.querySelectorAll('#gt-watch-list .gt-watch-item').forEach((item) => {
    const key = item.dataset.key;
    const btns = item.querySelectorAll('.gt-status-btn');
    const note = item.querySelector('.gt-watch-note');
    const stampEl = item.querySelector('.gt-watch-stamp');
    const rec = data[key] || {};
  ...
  activatePanel(getihuPanelId(deepLink.tab));

  return () => cleanups.forEach((fn) => fn());
}

export const initGetihu = withGyInit(initGetihuCore);
