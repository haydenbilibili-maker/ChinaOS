...
export function initXingshaoshu(root, deepLink = {}) {
  ...
  const nav = root.querySelectorAll('.xs-nav button');
  const activatePanel = (panelId) => {
    nav.forEach((b) => b.classList.toggle('is-active', b.dataset.panel === panelId));
    root.querySelectorAll('section.xs-panel').forEach((p) => {
  ...
  const stateEl = root.querySelector('#xs-storage-state');
  if (stateEl) {
  ...
  root.querySelectorAll('#xs-watch-list .xs-watch-item').forEach((item) => {
    const key = item.dataset.key;
    const btns = item.querySelectorAll('.xs-status-btn');
    const note = item.querySelector('.xs-watch-note');
    const stampEl = item.querySelector('.xs-watch-stamp');
    const rec = data[key] || {};
  ...
  activatePanel(xingshaoshuPanelId(deepLink.tab));
