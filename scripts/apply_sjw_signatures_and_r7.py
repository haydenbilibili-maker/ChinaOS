...
PAGES = ROOT / "app/src/modules/shijian-world"
...
def layout_hub_arc(nodes):
    pos = {"r1":(50,140),"r2":(160,95),"r3":(290,140),"r4":(420,175),"r5":(550,130),"r6":(680,95),
           "m13":(90,280),"m17":(320,280),"m23":(550,280)}
    sz = {k:(90,54) if k.startswith("r") else (180,70) for k in pos}
    parts = [E("arc1","M70 120 Q250 50 430 120",O,2), E("arc2","M430 120 Q610 190 750 110",C,2),
...
def layout_anchor_chain(nodes):
    pos = {"a1":(50,150,180,80),"a2":(310,150,180,80),"a3":(570,150,180,80),"gap":(200,280,400,60)}
    # map first 3 nodes to a1-a3 if needed - use node ids from catalog
    parts = [E("l1","M230 190 H310",LN,2,"4 3"),E("l2","M490 190 H570",V,2),E("l3","M400 230 V280",V,1.5)]
    # flexible: use positions by order
    keyed = {n[0]:n for n in nodes}
    coords = list(pos.items())
    for i,n in enumerate(nodes[:4]):
        key, (x,y,w,h) = coords[i]
        parts.append(N(n[0],x,y,w,h,n[1],n[2],COL[n[3]]))
    return "".join(parts)
